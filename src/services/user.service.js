...
const userAttributes = ['id', ['display_name', 'displayName'], 'email'];
...
const checkUser = async (email, password) => {
  ...
  if (!user || user.password !== password) {
    return { type: 'UNAUTHORIZED', message: 'invalid login credentials.' };
  }
...
const createUser = async (displayName, email, password) => {
  ...
  const newUser = await User.create(snakeize({ displayName, email, password }));
